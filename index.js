...
const keywords = [
  'commercial space station', 'cld', 'commercial low earth orbit',
  'commercial space', 'iss', 'international space station',
  'space station', 'human spaceflight', 'astronaut',
...
app.get('/', async function(req, res) {
  ...
  for (var f = 0; f < RSS_URLS.length; f++) {
    ...
    try {
      ...
      if (parsed.rss) {
        ...
        if (ch && ch<a href="" class="citation-link" target="_blank" style="vertical-align: super; font-size: 0.8em; margin-left: 3px;">[0]</a> && ch<a href="" class="citation-link" target="_blank" style="vertical-align: super; font-size: 0.8em; margin-left: 3px;">[0]</a>.item) {
          items = ch<a href="" class="citation-link" target="_blank" style="vertical-align: super; font-size: 0.8em; margin-left: 3px;">[0]</a>.item;
        }
      }
      if (!items) {
  ...
  var html = '<html><head><title>Space Articles</title>';
  html += '<style>';
  html += 'body{font-family:Arial,sans-serif;max-width:900px;margin:0 auto;padding:20px}';
  html += 'h1{color:#333}h2{color:#0056b3;border-bottom:2px solid #0056b3;padding-bottom:5px}';
  ...
  for (var s = 0; s < sourceNames.length; s++) {
    ...
    html += '<h2>' + name + ' (' + sources[name].length + ')</h2>';
    for (var k = 0; k < sources[name].length; k++) {
      var a = sources[name][k];
      html += '<div class="item">';
